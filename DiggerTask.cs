...
namespace Digger
{
    ...
    public class Monster : ICreature
    {
        ...
        public bool Conflict2(ICreature conflictedObject)
        {
            ...
            var a = conflictedObject.GetImageFileName();
            if (a == "Digger.png")
            //if (a == "Sack.png" || a == "Terrain.png" || a == "Monster.png")
                return true;
            return false;
        }
...
/*
            if (distanceFromXtoI <= distanceFromYtoJ )
                if (x + 1 < Game.MapWidth && x - i > 0 && CanMove(Game.Map[x+1,y]))
                    CrCmd.DeltaX = 1;
                else
                    if (x - 1 >= 0 && CanMove(Game.Map[x - 1, y]))
                        CrCmd.DeltaX = -1;
            else if (y + 1 < Game.MapHeight && y - j > 0 && CanMove(Game.Map[x, y+1]))
                    CrCmd.DeltaY = 1;
                 else if (y - 1 >= 0 && y - j > 0 && CanMove(Game.Map[x, y - 1]))
                    CrCmd.DeltaY = -1;
                    */
